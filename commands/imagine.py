...
class Cog2(commands.Cog):
  ...
  @app_commands.command(name="imagine",
                        description="Create an image with words.")
  ...
  async def imagine(self,
    ...
    try:
      ...
      embed = discord.Embed(title="ConvoAI Image Generation",
                            color=discord.Color.blue())
      embed.set_image(url="attachment://generated_image.png")
      embed.add_field(name="🖍️ Prompt", value=f'```{prompt}```', inline=True)
      embed.add_field(name="👤 User",
                      value=f'```{interaction.user.display_name}```',
                      inline=True)
      embed.add_field(name="🌱 Seed", value=f'```{seed}```', inline=True)
      embed.add_field(name="🔒 Privacy",
                      value="```Private```" if private else "```Public```",
                      inline=True)
      embed.set_footer(text="Made with ❤️ by ConvoAI team")

      buttons = ImageButtons(prompt, model.value, model.name, seed, private)
      buttons.generation_time = generation_time
      ...
      if private:
        ...
        message = await interaction.followup.send(file=image_file,
                                                  embed=embed,
                                                  view=buttons)
        buttons.original_message = message
    except Exception as e:
      await interaction.followup.send(
          f"An error occurred while generating the image: {e}", ephemeral=True)
